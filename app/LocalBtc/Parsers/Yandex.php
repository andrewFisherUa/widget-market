	...
	public function limites($lbtckey, $lbtcsecret, $limite){
		...
		if ($result->data->contact_count>0){
			foreach ($result->data->contact_list as $ads){
				...
				if ($ads->data->advertisement->payment_method=="YANDEXMONEY" and $ads->data->advertisement->trade_type=="ONLINE_BUY" and $ads->data->currency=="RUB"){
					$limite['new_buy']=$limite['new_buy']-$ads->data->amount;
				}
				if ($ads->data->advertisement->payment_method=="YANDEXMONEY" and $ads->data->advertisement->trade_type=="ONLINE_SELL" and $ads->data->currency=="RUB"){
