...
class OpenChat extends Model
{
	...
	public function NoMoney($lbtckey, $lbtcsecret, $contact_id){
	...
	public function index($lbtckey, $lbtcsecret, $contact_id, $summa, $reference){
		...
		Я робот Obmenneg! 
		Пришли мне номер счета кошелька в формате (+7...) без пробелов и запятых.
		Кошельки которые начинаются не на +7, такие как +3... и т.д. не принимаются.
	...
	public function find($lbtckey, $lbtcsecret, $contact_id){
		...
		if ($k>0){
			...
			$nonce=$int.$flo;
			$client_message="
			Не верный формат ввода, пожалуйста введите свой номер в формате (+7...).";
			$array_message=array(
				"msg"=>$client_message
			);
			$message=http_build_query($array_message);
	...
	public function otpusk($lbtckey, $lbtcsecret, $contact_id){
		...
		$nonce=$int.$flo;
		$client_message="
		Я перевел тебе " . $rob->amount . " руб. со счета:+79381148114
