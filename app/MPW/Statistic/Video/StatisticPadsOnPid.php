...
class StatisticPadsOnPid extends Model
{	
	...
	public function Group($data, $datetime, $userAgent, $country){
		...
		if (!isset($this->group[$data["page_key"]][$date][$data['pid']][$country][$device][$data['id_src']]['midpoint']))
		$this->group[$data["page_key"]][$date][$data['pid']][$country][$device][$data['id_src']]['midpoint']=0;
		if (!isset($this->group[$data["page_key"]][$date][$data['pid']][$country][$device][$data['id_src']]['third']))
		$this->group[$data["page_key"]][$date][$data['pid']][$country][$device][$data['id_src']]['third']=0;
		...
		$this->group[$data["page_key"]][$date][$data['pid']][$country][$device][$data['id_src']]['midpoint']++;
		if ($data['event']=='AdVideoThirdQuartile' or $data['event']=='AdVideoThirdQuartile ))')
		$this->group[$data["page_key"]][$date][$data['pid']][$country][$device][$data['id_src']]['third']++;
	...
	public function insert($data){
		...
		$sql="insert into statistic_pads_on_pid (page_key,date,pid,country,device,id_src,ad_started,started,played,midpoint,third,complete,click)
			select ?,?,?,?,?,?,?,?,?,?,?,?,? WHERE NOT EXISTS (SELECT 1 FROM statistic_pads_on_pid WHERE page_key=? and date=? and pid=? and country=? and device=? and id_src=?)";
		$sthInsert=$pdo->prepare($sql);
		$sql="update statistic_pads_on_pid set ad_started=?, started=?, played=?, midpoint=?, third=?, complete=?, click=?
					...
					foreach ($countrys as $country=>$devices){
						foreach ($devices as $device=>$id_srcs){
							foreach ($id_srcs as $id_src=>$events){
								...
								$events['complete'],$events['click'],$page_key,$date,$pid,$country,$device,$id_src]);
								$sthInsert->execute([$page_key,$date,$pid,$country,$device,$id_src,$events['AdStarted'],$events['started'],$events['played'],
								$events['midpoint'],$events['third'],$events['complete'],$events['click'],$page_key,$date,$pid,$country,$device,$id_src]);
							}
						}
				}
				}
	...
	public function isertStat($date){
		$pdo = \DB::connection('videostatistic')->getPdo();
		$sql="SELECT date, country, device, pid, id_src, count(page_key) as requested, sum(ad_started) as ad_started, sum(started) as started, 
		...
		from statistic_pads_on_pid where date='$date' group by date,pid,id_src,country,device";
		$data=$pdo->query($sql, \PDO::FETCH_ASSOC)->fetchALL(\PDO::FETCH_ASSOC);
		$pdo = \DB::connection()->getPdo();
		$sql="insert into video_statistic_pads_on_pid_new_player (date,country,device,pid,id_src,requested,ad_started,started,played,completed,clicked,
		poteri,dosm,util,ctr) select ?,?,?,?,?,?,?,?,?,?,?,?,?,?,? WHERE NOT EXISTS (SELECT 1 FROM video_statistic_pads_on_pid_new_player WHERE date=? 
		and country=? and device=? and pid=? and id_src=?)";
		$sthInsert=$pdo->prepare($sql);
		$sql="update video_statistic_pads_on_pid_new_player set requested=?, ad_started=?, started=?, played=?, completed=?, clicked=?, poteri=?,
		dosm=?,	util=?, ctr=? WHERE date=? and country=? and device=? and pid=? and id_src=?";
		$sthUpdate=$pdo->prepare($sql);
		foreach ($data as $pad){
			$sthUpdate->execute([$pad['requested'],$pad['ad_started'],$pad['started'],$pad['played'],$pad['completed'],$pad['clicked'],$pad['poteri'],
			$pad['dosm'],$pad['util'],$pad['ctr'],$pad['date'],$pad['country'],$pad['device'],$pad['pid'],$pad['id_src']]);
			$sthInsert->execute([$pad['date'],$pad['country'],$pad['device'],$pad['pid'],$pad['id_src'],$pad['requested'],$pad['ad_started'],$pad['started'],
			$pad['played'],$pad['completed'],$pad['clicked'],$pad['poteri'],$pad['dosm'],$pad['util'],$pad['ctr'],$pad['date'],$pad['country'],
			$pad['device'],$pad['pid'],$pad['id_src']]);
		}
	}
