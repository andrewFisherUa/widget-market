...
class Banks_two extends Command
{
     ...
     * @var string
     */
    protected $signature = 'LocalBtc:banks_two';
     ...
     */
	//для сделок 626956
	private $lbtckey='1106d46d84369a5f87a472d9f71319d9'; //ключ localbitcoins parsers_banks_two
	private $lbtcsecret='5cb9c3841488e250315619a575411928035536851afe1113ce66b654a6b5d708'; //секрет localbitcoins parsers_banks_two
    public function handle()
    {
	
		$ads=\DB::connection('obmenneg')->table('all_ads')->whereIn('id_ads', ['626956'])->get();
		$pdo = \DB::connection("obmenneg")->getPdo();
		$sql="insert into banks_offers_two_tmp (id_ads,username,name,temp_price,my_id_ad,min_amount,max_amount)
			select ?,?,?,?,?,?,?";
		...
		foreach ($ads as $ad){
			...
			foreach ($result->data->ad_list as $k=>$res){
				...
				if ($ad->parse_status){
					if ($ad->position-1==$i){
						\App\Lbtc\Parsers\Banks_two::getInstance()->index($ad->id_ads, $this->lbtckey, $this->lbtcsecret, $res->data->temp_price);
					}
				}
				else{
					if ($i==0){
						\App\Lbtc\Parsers\Banks_two::getInstance()->indexOff($ad->id_ads, $this->lbtckey, $this->lbtcsecret);
					}
				}
		...
		$sql='
		ALTER TABLE banks_offers_two RENAME TO banks_offers_two_temp;
		ALTER TABLE banks_offers_two_tmp RENAME TO banks_offers_two;
		ALTER TABLE banks_offers_two_temp RENAME TO banks_offers_two_tmp;
		truncate TABLE banks_offers_two_tmp;
		';
		$pdo->exec($sql);
	}
}
