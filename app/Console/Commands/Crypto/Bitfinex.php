...
class Bitfinex extends Command
{
     ...
     * @var string
     */
    protected $signature = 'Crypto:bitfinex';
...
return;
		...
		$sql="insert into transactions (market,from_valut,to_valut,volume,volume_to_usd,volume_to_rub,price,price_to_usd,price_to_rub,datetime,amount_from,amount_to)
			select ?,?,?,?,?,?,?,?,?,?,?,? WHERE NOT EXISTS (SELECT 1 FROM transactions WHERE market=? and from_valut=? and to_valut=?)";
		$sthInsert=$pdo->prepare($sql);
		$sql="update transactions set volume=?, volume_to_usd=?, volume_to_rub=?, price=?, price_to_usd=?, price_to_rub=?, datetime=?, amount_from=?, amount_to=?
