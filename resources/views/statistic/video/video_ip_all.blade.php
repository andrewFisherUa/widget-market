...
<div class="container">
	<div class="row">
		...
		<div class="row">
			...
			<div class="row">
				<h4 class="text-center">Стата по ip </h4>
				<div class="col-xs-12">
					<div>
						...
						<table class="table table-hover table-bordered" style="margin-top: 10px; table-layout: fixed;">
							...
							<tbody>
								@foreach ($stats as $stat)
									<tr>
										<td>{{$stat->ip}}</td>
										<td><a href="{{route('video_statistic.group.ip', ['id'=>$stat->pid])}}" style="font-weight: bold" target="_blank">{{$stat->pid}}</a></td>
										<td><a href="{{route('admin.home', ['user_id'=>$stat->user_id])}}" style="font-weight: bold" target="_blank">{{$stat->name}}</a></td>
										<td>{{$stat->domain}}</td>
