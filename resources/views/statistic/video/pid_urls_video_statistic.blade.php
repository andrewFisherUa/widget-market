	...
	<div class="row">
		...
		<div class="row">
			...
			<div class="row">
				...
				@endif
				<h4 class="text-center">Детализация по url адресам виджета № {{$widget->id}} {{\App\PartnerPad::where('id', $widget->widget->pad)->first()->domain}}</h4>
				<div class="col-xs-12">
					<div>
						<table class="table table-hover table-bordered" style="margin-top: 10px; table-layout: fixed;">
							<thead>
								<colgroup>
									<col span="1" style="width: 569px">
							...
							<tbody>
								@foreach ($stats as $stat)
									<tr>
										...
										<td>{{$stat->played}}</td>
										<td>{{$stat->calculate}}</td>
										<td>{{$stat->percent_requested}}</td>
										<td>{{$stat->percent_played}}</td>
										<td>{{$stat->percent_calculate}}</td>
									</tr>
								@endforeach
							</tbody>
						</table>
