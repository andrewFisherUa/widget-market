...
<div class="container">
	...
	<div class="row">
		...
		<div class="row">
			...
			<div class="row">
				<h4 class="text-center">Cтатистика по площадке <a href="{{route('pads.edit', ['id'=>$pad->id])}}" target="_blank">{{$pad->domain}}</a> по товарным виджетам в период с {{date('d-m-Y',strtotime($from))}} по {{date('d-m-Y',strtotime($to))}}</h4>
				<!--{{$profile=$pad->userProfile($pad->user_id)}}-->
				Юзер: <a href="{{route('admin.home', ['user_id'=>$profile->user_id])}}" target="_blank">{{$profile->name}}</a><br>
				@if ($profile->manager)
				Менеджер: <a href="{{route('admin.home', ['user_id'=>$profile->manager])}}" target="_blank">{{\App\User::find($profile->manager)->name}}</a>
				@endif
				<div class="col-xs-12">
