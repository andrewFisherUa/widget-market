...
<div class="container">
	...
	<div class="row">
		<h3 class="text-center">Обмены по Yandex</h3>
			...
			<tbody>
				<tr style="background: #000; color: #fff">
					...
					<td>
						@php
							...
							$sql="select * from local_balancing where id_ad='609928'";
					...
					<td>
						@php
							$pdo = \DB::connection("obmenneg")->getPdo();
							$sql="select * from local_balancing where id_ad='609305'";
							$srs=$pdo->query($sql, \PDO::FETCH_ASSOC)->fetchAll(\PDO::FETCH_ASSOC);
							$course_all_sell=0;
							foreach ($srs as $sr){
				...
				@foreach ($stats as $stat)
					<tr>
						...
						@php
							$pdo = \DB::connection("obmenneg")->getPdo();
							$sql="select * from local_balancing where id_ad='609928' and date_trunc('day', created)='" . $stat['day'] . "'";
							$srs=$pdo->query($sql, \PDO::FETCH_ASSOC)->fetchAll(\PDO::FETCH_ASSOC);
							$course=0;
							$details_buy='';
							$course_null=0;
						...
						@php
							$pdo = \DB::connection("obmenneg")->getPdo();
							$sql="select * from local_balancing where id_ad='609305' and date_trunc('day', created)='" . $stat['day'] . "'";
							$srs=$pdo->query($sql, \PDO::FETCH_ASSOC)->fetchAll(\PDO::FETCH_ASSOC);
							$course=0;
							$course_null=0;
						...
						<td>{{$stat['profit']}}</td>
						<td><a href="{{route('lbtc.yandex.robot.list.v3.detail', ['date'=>$stat['day']])}}" target="_blank" style="display: block">Подробнее</a></td>
					</tr>
				@endforeach
				<tr style="background: #000; color: #fff">
					<td>Всего</td>
