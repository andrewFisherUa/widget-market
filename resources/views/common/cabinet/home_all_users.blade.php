	...
	@foreach ($stats as $userActive)
		<tbody>
			<tr>
				...
				<td>
					<a href="{{route('admin.home', ['user_id'=>$userActive->user_id])}}" target="_blank" style="color: #636b6f;">{{$userActive->name}} @if ($userActive->vip==1)<img src="/images/cabinet/vip.png" data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="VIP клиент" style="width: 20px; position: relative; top: -3px; cursor: pointer;">@endif @if ($userActive->lease==1)
						<span data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="На аренде" style="color: #ffffff; font-weight: bold;     text-shadow: 0 0 10px #0000ff, 0 0 15px #0000ff, 0 0 20px #0000ff, 0 0 25px #0000ff, 0 0 30px #0000ff, 0 0 35px #0000ff, 0 0 40px #0000ff, 0 0 45px #0000ff; margin-left: 5px;">А</span>
					@endif</a>
				...
				<td colspan='6'>
					@if ($userActive->dop_status==1)
						<img src="/images/smail/green.png" data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="{{$userActive->text_for_dop_status}}" style="width: 20px; height: 20px; display: inline-block; cursor: pointer; top: -4px; position: relative;">
					@elseif ($userActive->dop_status==2)
						<img src="/images/smail/yellow.png" data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="{{$userActive->text_for_dop_status}}" style="width: 20px; height: 20px; display: inline-block; cursor: pointer; top: -4px; position: relative;">
					@elseif ($userActive->dop_status==3)
						<img src="/images/smail/red.png" data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="{{$userActive->text_for_dop_status}}" style="width: 20px; height: 20px; display: inline-block; cursor: pointer; top: -4px; position: relative;">
					@endif
					...
					@if ($userActive->status==1)
						...
						<!--<a href="{{route('admin.user_active', ['id_user'=>$userActive->user_id])}}" data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="Отметить как активный клиент" style="float: right"><span class="glyphicon glyphicon-eye-open color-green"></span></a>-->
					@else
						...
						<!--<a href="{{route('admin.user_no_active', ['id_user'=>$userActive->user_id])}}" data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="Отметить как неактивный клиент" style="float: right"><span class="glyphicon glyphicon-eye-close color-red"></span></a>-->
					@endif
				</td>
			</tr>
		</tbody>
...
@foreach ($stats as $userActive)
	@include('common.cabinet.modal.add_user_site')
	@include('common.cabinet.modal.add_user_widget')
	@include('common.cabinet.modal.add_user_dop_status')
	@if (\Auth::user()->hasRole('admin'))
		@include('common.cabinet.modal.add_default_on_users')
	@endif
@endforeach
{!! $stats->appends(["direct"=>$direct, "order"=>$order])->render() !!}