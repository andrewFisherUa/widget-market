...
@if ($user->profile->lease!=1 or \Auth::user()->hasRole('admin') or \Auth::user()->hasRole('manager') or \Auth::user()->hasRole('super_manager'))
...
@include('common.cabinet.modal.payment')
@include('common.cabinet.modal.auto_payment')
@else
<p class="text-center">Вы отмечены как юзер с постоянной установленой оплатой, поэтому Вам не начислятеся баланс и не доступны некоторые функции в этом блоке.</p>
@endif