<tr>
	<td colspan="18">
		<div class="col-xs-2" style="margin: 5px 0;">
			<button data-toggle="modal" data-target="#add_user_site_video_{{$id}}" type="submit" class="btn btn-success">
				Добавить площадку
			</button>
		</div>
		<div class="col-xs-2" style="margin: 5px 0;">
			<button data-toggle="modal" data-target="#add_user_widget_video_{{$id}}" type="submit" class="btn btn-success">
				Создать виджет
			</button>
		</div>
		<div class="col-xs-2" style="margin: 5px 0;">
			<button data-toggle="modal" data-target="#add_user_dop_status_video_{{$id}}" type="submit" class="btn btn-success">
				Изменить доп. статус
			</button>
		</div>
		@if (\Auth::user()->hasRole('admin'))
			<div class="col-xs-2" style="margin: 5px 0;">
				<button data-toggle="modal" data-target="#add_default_on_users_video_{{$id}}" type="submit" class="btn btn-success">
		...
		@if (\Auth::user()->hasRole('admin'))
			<div class="col-xs-3" style="margin: 5px 0;">
				@if ($user->lease==0)
				<button type="submit" data-set="{{$id}}" class="user_lease btn btn-success">
					Отметить как арендный
				</button>
				@else
				<button type="submit" data-set="{{$id}}" class="user_no_lease btn btn-success">
					Снять с арендны
				</button>
				@endif
			</div>
		@endif
		<div class="col-xs-7" style="margin: 5px 0;">
			@if ($frame)
				<a href="{{route('video_statistic.frame_prov', ['id'=>$user->user_id])}}" class="btn btn-success">Продлить на сутки</a>
				<span style="margin: 0 2px;">Выключится: {{$frame->datetime}}</span>
			@else
				<a href="{{route('video_statistic.frame_prov', ['id'=>$user->user_id])}}" class="btn btn-success">Включить проверку</a>
			@endif
			<a target="_blank" href="{{route('video_statistic.frame_stat_user', ['id'=>$user->user_id])}}" class="btn btn-success">Статистика по фреймам</a>
		</div>
	</td>
</tr>
<tr>
	...
	<td class="text-center">Б. за глубину</td>
...
@foreach ($widgets as $widget)
	<tr>
		<td>
			@if ($widget->type==1)
				<span data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="Товарный виджет" class="glyphicon glyphicon glyphicon-shopping-cart affiliate_all_pads_domain_gliph green widget-gl"></span>
			@elseif ($widget->type==2)
				@if ($widget->video['type']==1)
					<span data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="Автоплей виджет" class="glyphicon glyphicon glyphicon glyphicon-facetime-video affiliate_all_pads_domain_gliph green widget-gl"></span>
				@elseif ($widget->video['type']==2)
					<span data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="Оверлей виджет" class="glyphicon glyphicon glyphicon glyphicon-facetime-video affiliate_all_pads_domain_gliph green widget-gl"></span>
				@elseif ($widget->video['type']==3)
					<span data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="Vast" class="glyphicon glyphicon glyphicon glyphicon-facetime-video affiliate_all_pads_domain_gliph green widget-gl"></span>
				@elseif ($widget->video['type']==4)
					<span data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="InPage виджет" class="glyphicon glyphicon glyphicon glyphicon-facetime-video affiliate_all_pads_domain_gliph green widget-gl"></span>
		...
		<td style="position: relative;" class="erty">
			@if ($widget->type==1)
			...
			@elseif ($widget->type==2)
				...
				@endif
				<!--{{($tool=$widget->VideoTooltip($widget->video['id']))?"":""}}-->
		...
		<td class="text-center">
			...
			@elseif ($widget->type==2)
				...
				@endif
				@if ($widget->video['control_rus']==1 or $widget->video['control_cis']==1)
				<span style="color: #a51313;font-weight: bold;">K</span>
				@else
				<span>{{round($widget->videoCommisssion($widget->video['commission_rus']),0)}} и {{round($widget->videoCommisssion($widget->video['commission_cis']),0)}}</span>
				@endif
				</td>
			@elseif ($widget->type==3)
			...
			@elseif ($widget->type==2)
				...
				<td class="text-center"><a href="{{ route('video_statistic.group.ip', ['id'=>$widget->video['id']])}}" target="_blank" data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="ip показов" style="color: red; font-weight: bold; text-decoration: none;">IP</a></td>
			@elseif ($widget->type==3)
				<!--{{($statistic=$widget->ProductStat($widget->id, $widget->tizer['id'], $from, $to))?"":""}}-->
				<td class="text-center"></td>
				<td class="text-center">{{$statistic?$statistic['loaded']:0}}</td>
