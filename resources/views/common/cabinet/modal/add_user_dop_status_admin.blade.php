<div id="add_user_dop_status" class="modal fade">
		...
		<div class="affiliate_modal_header">Назначение дополнительного статуса<button class="modal_exit glyphicon glyphicon-remove-sign" type="button" data-dismiss="modal" data-toggle="tooltip" data-placement="bottom" title="Закрыть"></button></div>
			...
			<form class="form-horizontal" role="form">
				...
				<div class="form-group">
					...
					<div class="col-xs-6">
						<input name="dop_status" @if ($user->profile->dop_status==1) checked @endif value="1" type="radio" style="margin-top: 12px">
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-xs-6">
						<input name="dop_status" @if ($user->profile->dop_status==2) checked @endif value="2" type="radio"  style="margin-top: 12px">
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-xs-6">
						<input name="dop_status" @if ($user->profile->dop_status==3) checked @endif value="3" type="radio"  style="margin-top: 12px">
					</div>
				</div>
				<div class="form-group">
					<label for="dop_status" class="col-xs-6 control-label">Без отметки</label>
					<div class="col-xs-6">
						<input name="dop_status" @if (!$user->profile->dop_status) checked @endif value="0" type="radio"  style="margin-top: 12px">
					</div>
				</div>
				<div class="form-group">
					<label for="text_for_dop_status" class="col-xs-4 control-label">Сопроводительный коментарий</label>
					<div class="col-xs-6">
						<textarea name="text_for_dop_status" class="form-control" style="resize: none;">{{$user->profile->text_for_dop_status}}</textarea>
					</div>
				</div>
				<div class="form-group">
					<div class="col-xs-ffset-1 col-xs-10 text-center">
					  <a data-set="{{$user->id}}" id="user_dop_status_submit" class="btn btn-primary">Сохранить</a>
					</div>
				</div>
			</form>
		</div>
