<div id="add_default_on_users" class="default_commission modal fade">
	<div class="modal-dialog">
		...
		<div class="affiliate_modal_header">Назначение особенных комиссий<button class="modal_exit glyphicon glyphicon-remove-sign" type="button" data-dismiss="modal" data-toggle="tooltip" data-placement="bottom" title="Закрыть"></button></div>
			<hr class="modal_hr">
			<!--{{$commissions=\App\VideoDefaultOnUser::where('user_id', $user->id)->get()}}-->
			<!--{{$Productcommissions=\App\ProductDefaultOnUser::where('user_id', $user->id)->get()}}-->
			<!--{{$controlcommissions=\App\UserLinkSumma::where('user_id', $user->id)->get()}}-->
			...
			<form class="form-horizontal" role="form">
				...
				<div class="form-group commission_save" style="display: none;">
					<div class="col-xs-offset-1 col-xs-10 text-center">
					  <a data-set="{{$user->id}}" id="default_submit" class="btn btn-primary">Сохранить</a>
					</div>
				</div>
			</form>
		</div>
