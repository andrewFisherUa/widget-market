<div id="auto_payment" data-set="{{$userProf->vip}}" class="modal fade">
	<div class="modal-dialog">
		...
		<div class="affiliate_modal_header">Настройка автозаказа выплат<button class="modal_exit glyphicon glyphicon-remove-sign" type="button" data-dismiss="modal" data-toggle="tooltip" data-placement="bottom" title="Закрыть"></button></div>
			...
			@if ($all_options>0)
			<form class="form-horizontal" role="form" method="post" action="{{ route('user_payout_auto')}}">
				{!! csrf_field() !!}
				...
				<div class="form-group">
					...
					<div class="col-xs-6">
						<input type="checkbox" name="auto_pay" value="1" @if ($userProf->auto_payment) checked @endif style="margin-top: 12px;">
					</div>
				</div>
				<!-- {{$autoPay=\App\Payments\UserPaymentAuto::where('user_id', $userProf->user_id)->first()}} -->
				<div class="form-group">
