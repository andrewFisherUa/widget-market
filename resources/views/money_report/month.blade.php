...
<div class="container">
    ...
    <div class="row text-center">
		...
		<h3>Детализация {{$account->title}} за {{$mo}}</h3>
		...
		@if ($month==13)
			@for ($i = 1; $i <= $l; $i++)
				<!--{{($stats=$account->yearStat($account->id, $i, $year))?"":""}}-->
					<!--@if ($i=="1")
						{{$mm="Январь"}}
					@elseif ($i=="2")
						{{$mm="Февраль"}}
				...
				@if ($i==1 or $i==7)
					<div class="col-xs-2" style="border: solid 1px #000; padding: 0; margin: 2px 0;">
						...
						<div class="col-xs-6" style="padding: 0; font-weight: bold; @if(round($stats?$stats['plus']:0,8)>0) color: green; @endif">
						@if ($account->shortcode=='btc' || $account->shortcode=='ltc')
						{{round($stats?$stats['plus']:0,8)}}
						@else
						{{round($stats?$stats['plus']:0,2)}}
						@endif
						</div>
						<div class="col-xs-6" style="padding: 0; font-weight: bold; @if(round($stats?$stats['minus']:0,8)>0) color: red; @endif border-left: 1px solid #999;">
						@if ($account->shortcode=='btc' || $account->shortcode=='ltc')
						{{round($stats?$stats['minus']:0,8)}}
						@else
						{{round($stats?$stats['minus']:0,2)}}
						@endif
						</div>
					</div>
				@else
					<div class="col-xs-2" style="border: solid 1px #000; border-left: 0; padding: 0; margin: 2px 0;">
						...
						<div class="col-xs-6" style="padding: 0; font-weight: bold; border-left: 1px solid #999; @if(round($stats?$stats['minus']:0,8)>0) color: red; @endif">
						@if ($account->shortcode=='btc' || $account->shortcode=='ltc')
						{{round($stats?$stats['minus']:0,8)}}
						@else
						{{round($stats?$stats['minus']:0,2)}}
		...
		@else
			@for ($i = 1; $i <= $l; $i++)
				...
				{{$day=$year . '-' . $month . '-' . $g . ''}}
				-->
				<!--{{($stats=$account->monthStat($account->id, $i, $month, $year))?"":""}}-->
				<!--{{$balance=\App\MoneyReport\ReportAccount::where('report_id', $stats['report'])->where('account_id', $account->id)->first()}}-->
				@if ($i==1 or $i==7 or $i==13 or $i==19 or $i==25 or $i==31)
					<div class="col-xs-2" style="border: solid 1px #000; padding: 0; margin: 2px 0;">
						<div class="col-xs-12" style="border-bottom: solid 1px #999"><a target="_blank" href="{{route('money.operation.report.account', ['report_id'=>$stats['report'], 'account_id'=>$account->id])}}"><strong>{{$i}}.{{$month}}</strong></a>
						@if($balance) {{$balance->summa_opened}} @endif
						</div>
						<div class="col-xs-6" style="padding: 0; border-bottom: solid 1px #999" >+</div>
						<div class="col-xs-6" style="padding: 0; border-left: 1px solid #999; border-bottom: solid 1px #999;">-</div>
						<div class="col-xs-6" style="padding: 0; font-weight: bold; @if(round($stats?$stats['plus']:0,8)>0) color: green; @endif">
